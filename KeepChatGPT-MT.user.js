...
// @version           1.0
// @author            MTam
// @namespace         https://github.com/JustaTama/KeepChatGPT-MT/
// @supportURL        https://github.com/JustaTama/KeepChatGPT-MT/
...
// @noframes
// @downloadURL https://raw.githubusercontent.com/JustaTama/KeepChatGPT-MT/main/KeepChatGPT-MT.user.js
// @updateURL https://raw.githubusercontent.com/JustaTama/KeepChatGPT-MT/main/KeepChatGPT.user-MT.js
// ==/UserScript==
...
"vi": {"dm": "Chế độ tối", "sd": "Show debugging", "cm": "Huỷ kiểm toán", "ca": "Hủy hoạt hình", "ab": "About", "si": "Đề xuất khoảng thời gian 50 giây", "mi": "Sửa khoảng cách", "cu": "Kiểm tra cập nhật", "cv": "Phiên bản hiện tại", "dl": "Discover the latest version", "lv": "là phiên bản mới nhất", "cc": "Sao chép cuộc trò chuyện", "pp": "Làm sạch trang", "ls": "Hiển thị màn hình lớn", "sc": "Nói đầy đủ", "it": "Chặn Theo Dõi", "ec": "More chat info", "ap": "Đánh giá", "wn": "Cảnh báo", "ds": "Bảo mật dữ liệu", "dd": "Phát hiện dữ liệu nhạy cảm", "rr": "Sử dụng regex để viết quy tắc", "ko": "Quan sát tinh tế"},
    ...
    const setIfr = function(u = "") {
        if ($("#MTam") === null) {
            ...
            nIfr.onload = function() {
                const nIfrText = $("#MTam").contentWindow.document.documentElement.innerText;
                try {
                    $("#MTam").contentWindow.document.documentElement.style = `background: #FCF3CF; height: 360px; width: 1080px; overflow; auto;`;
                    if (nIfrText.indexOf(`"expires":"`) > -1) {
                        console.log(`KeepChatGPT: IFRAME: Expire date: ${formatDate(JSON.parse(nIfrText).expires)}`);
                        $("#MTam").contentWindow.document.documentElement.innerHTML = formatJson(nIfrText);
            ...
            onload: function(response) {
                ...
                try {
                    if (response.responseHeaders.match(/content-type:\s*application\/json/i) && response.status !== 403 && data.indexOf(`"expires":"`) > -1) {
                        ...
                        //$("#MTam").contentWindow.document.documentElement.innerHTML = formatJson(data);
                    } else {
                        setIfr(u);
                    }
                } catch (e) {
...
`;
    ...
    const loadKCG = function() {
        ...
        ndivkcg._symbol1_innerHTML = `<img src='${icon}' style='width: 1rem;' /><div style='font-size: 0.8rem'>KeepChatGPT by MTam</div>`;
...
`;
    ...
    const checkForUpdates = function(action = "click") {
        const downloadURL = `https://raw.githubusercontent.com/JustaTama/KeepChatGPT-MT/main/KeepChatGPT-MT.user.js`;
